 ...
 * We store the intermediate reduction of each subarray (following upsweep_pow2)
 * in [part]. These partial values can themselves be scanned and fed into
 * [scan_inc_subarrays].
 */
__kernel void scan_subarrays(__global int *data, // length [n]
...
/*
 * Perform the second phase of an inplace exclusive scan on a global array
 * [data] of arbitrary length [n].
 *
 * We assume that we have k workgroups each of size m/2 workitems.
 * Each workgroup handles a subarray of length [m] (where m is a power of two).
 * We sum each element by the sum of the preceding subarrays taken from [part].
 */
__kernel void scan_inc_subarrays(__global int *data, // length [n]
                                 __local int *x,     // length [m]
                                 __global int *part, // length [m]
                                 int n
#if DEBUG
