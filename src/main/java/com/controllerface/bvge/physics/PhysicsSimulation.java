...
public class PhysicsSimulation extends GameSystem
{
    ...
    private final GPUProgram p_origin_shift             = new OriginShift();
    ...
    private final GPUKernel k_shift_points;
    private final GPUKernel k_shift_hulls;
    private final GPUKernel k_shift_entities;
    ...
    public PhysicsSimulation(ECS ecs, UniformGrid uniform_grid, PlayerController player_controller)
    {
        ...
        long k_ptr_shift_points = p_origin_shift.kernel_ptr(Kernel.shift_points);
        k_shift_points = new ShiftPoints_k(GPGPU.ptr_compute_queue, k_ptr_shift_points)
            .buf_arg(ShiftPoints_k.Args.points, GPGPU.core_memory.get_buffer(CoreBufferType.POINT));

        long k_ptr_shift_hulls = p_origin_shift.kernel_ptr(Kernel.shift_hulls);
        k_shift_hulls = new ShiftHulls_k(GPGPU.ptr_compute_queue, k_ptr_shift_hulls)
            .buf_arg(ShiftHulls_k.Args.hulls, GPGPU.core_memory.get_buffer(CoreBufferType.HULL));

        long k_ptr_shift_entities = p_origin_shift.kernel_ptr(Kernel.shift_entities);
        k_shift_entities = new ShiftEntities_k(GPGPU.ptr_compute_queue, k_ptr_shift_entities)
            .buf_arg(ShiftEntities_k.Args.entities, GPGPU.core_memory.get_buffer(CoreBufferType.ENTITY));
    ...
    {
        long local_buffer_size = cl_int.size() * GPGPU.max_scan_block_size;

        GPGPU.cl_zero_buffer(GPGPU.ptr_compute_queue, svm_atomic_counter, cl_int.size());
    ...
    private void shift_origins(boolean unshift)
    {
        int point_count  = GPGPU.core_memory.sector_container().next_point();
        int hull_count   = GPGPU.core_memory.sector_container().next_hull();
        int entity_count = GPGPU.core_memory.sector_container().next_entity();
        ...
        float x_shift = unshift ? -uniform_grid.x_origin() : uniform_grid.x_origin();
        float y_shift = unshift ? -uniform_grid.y_origin() : uniform_grid.y_origin();

        k_shift_points
            .set_arg(ShiftPoints_k.Args.max_point, point_count)
            .set_arg(ShiftPoints_k.Args.x_shift, x_shift)
            .set_arg(ShiftPoints_k.Args.y_shift, y_shift)
            .call(arg_long(point_size), GPGPU.preferred_work_size);

//        k_shift_hulls
//            .set_arg(ShiftHulls_k.Args.max_hull, hull_count)
//            .set_arg(ShiftHulls_k.Args.x_shift, x_shift)
//            .set_arg(ShiftHulls_k.Args.y_shift, y_shift)
//            .call(arg_long(hull_size), GPGPU.preferred_work_size);

//        k_shift_entities
//            .set_arg(ShiftEntities_k.Args.max_entity, entity_count)
//            .set_arg(ShiftEntities_k.Args.x_shift, x_shift)
//            .set_arg(ShiftEntities_k.Args.y_shift, y_shift)
//            .call(arg_long(entity_size), GPGPU.preferred_work_size);
    }
