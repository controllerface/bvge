...
public class PhysicsObjects
{
    ...
    public static int particle(float x, float y, float size, float mass, float friction, float restitution)
    {
        ...
        int hull_id = GPGPU.core_memory.new_hull(mesh.mesh_id(), transform, rotation,
                table, bone_table, friction, restitution,
                next_armature_id, hull_flags);
    ...
    public static int wrap_model(int model_index, float x, float y, float size, int global_hull_flags, float mass, float friction)
    {
        ...
        for (int mesh_index = 0; mesh_index < meshes.length; mesh_index++)
        {
            ...
            int hull_id = GPGPU.core_memory.new_hull(hull_mesh.mesh_id(), transform, rotation,
                    table, bone_table, friction, 0,
                    next_armature_id, flag_bits);
