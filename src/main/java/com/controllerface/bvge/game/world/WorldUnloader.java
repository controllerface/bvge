    ...
    private void unload_sectors(int[] last_counts)
    {
        ...
        if (entity_count > 0)
        {
            ...
            for (int entity_offset = 0; entity_offset < entity_count; entity_offset++)
            {
                ...
                var entity_anim_elapsed_x  = raw_sectors.entity_anim_elapsed[entity_2_x];
        ...
        if (broken_count > 0)
        {
            ...
            for (int type : raw_broken.uv_offsets)
            {
                ...
                if (substance instanceof Solid solid)
                {
                    ...
                    {
                        batch.new_block(true, x - offset, y - offset, sz, 90, 0, 0, 0, Constants.HullFlags.COLLECTABLE._int, solid, new int[4]);
                        batch.new_block(true, x - offset, y + offset, sz, 90, 0, 0, 0, Constants.HullFlags.COLLECTABLE._int, solid, new int[4]);
                        batch.new_block(true, x + offset, y - offset, sz, 90, 0, 0, 0, Constants.HullFlags.COLLECTABLE._int, solid, new int[4]);
                    ...
                    {
                        batch.new_shard(false, false, x - offset, y + offset, sz, 0, Constants.HullFlags.COLLECTABLE._int, 30, 0, 0, solid);
                        batch.new_shard(false, true, x + offset, y + offset, sz, 0, Constants.HullFlags.COLLECTABLE._int, 30, 0, 0, solid);
                    }
                    ...
                    {
                        batch.new_shard(true, false, x - offset, y + offset, sz, 0, Constants.HullFlags.COLLECTABLE._int, 30, 0, 0, solid);
                        batch.new_shard(true, false, x + offset, y - offset, sz, 0, Constants.HullFlags.COLLECTABLE._int, 30, 0, 0, solid);
                        batch.new_shard(true, false, x + offset, y + offset, sz, 0, Constants.HullFlags.COLLECTABLE._int, 30, 0, 0, solid);
                    }
                }
