...
public class UpdateMousePosition_k extends GPUKernel
{
    ...
    public UpdateMousePosition_k(CL_CommandQueue command_queue_ptr, GPUProgram program)
    {
        super(command_queue_ptr, program.get_kernel(KernelType.update_mouse_position));
    }

    public GPUKernel init(CoreBufferGroup core_buffers)
    {
        return this.buf_arg(Args.entity_root_hulls, core_buffers.buffer(ENTITY_ROOT_HULL))
