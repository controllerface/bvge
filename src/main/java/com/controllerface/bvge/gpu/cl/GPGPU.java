...
public class GPGPU
{
    ...
    public static long cl_new_pinned_int()
    {
        try (var stack = MemoryStack.stackPush())
        {
            var status = stack.mallocInt(1);
            long flags = CL_MEM_HOST_READ_ONLY | CL_MEM_ALLOC_HOST_PTR;
            long ptr = CL10.clCreateBuffer(ptr_context, flags, CL_DataTypes.cl_int.size(), status);
            int result = status.get(0);
            if (result != CL_SUCCESS)
            {
    ...
    public static long cl_new_unpinned_int()
    {
        ...
        {
            var status = stack.mallocInt(1);
            long ptr = CL10.clCreateBuffer(ptr_context, CL_MEM_HOST_READ_ONLY, CL_DataTypes.cl_int.size(), status);
            int result = status.get(0);
            if (result != CL_SUCCESS)
            {
    ...
    {
        return clSVMAlloc(ptr_context, CL_MEM_READ_WRITE, CL_DataTypes.cl_int.size(), 0);
    }
