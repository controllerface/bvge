...
public class SectorGroup extends BufferGroup
{
    public SectorGroup(String name, long ptr_queue, long entity_init, long hull_init, long edge_init, long point_init)
    {
         ...
         */
        set_buffer(POINT_BONE_TABLE, new_buffer(cl_int4, point_init));
         ...
         */
        set_buffer(POINT, new_buffer(cl_float4, point_init));

        /* int
         * x: reference vertex index
         */
        set_buffer(POINT_VERTEX_REFERENCE, new_buffer(cl_int, point_init));

        /* int
         * x: hull index
         */
        set_buffer(POINT_HULL_INDEX, new_buffer(cl_int, point_init));

        /* int
         * x: vertex flags (bit field)
         */
        set_buffer(POINT_FLAG, new_buffer(cl_int, point_init));

        /* ushort
         * x: recent collision hit counter
         */
        set_buffer(POINT_HIT_COUNT, new_buffer(cl_short, point_init));
         ...
         * y: point 2 index
         */
        set_buffer(EDGE, new_buffer(cl_int2, edge_init));

        /* float
         * x: edge constraint length
         */
        set_buffer(EDGE_LENGTH, new_buffer(cl_float, edge_init));

        /* int
         * x: edge flags (bit-field)
         */
        set_buffer(EDGE_FLAG, new_buffer(cl_int, edge_init));
         ...
         * w: previous y position
         */
        set_buffer(HULL, new_buffer(cl_float4, hull_init));

        /* float2
         * x: scale x
         * y: scale y
         */
        set_buffer(HULL_SCALE, new_buffer(cl_float2, hull_init));

        /* int2
         * x: start point index
         * y: end point index
         */
        set_buffer(HULL_POINT_TABLE, new_buffer(cl_int2, hull_init));

        /* int2
         * x: start edge index
         * y: end edge index
         */
        set_buffer(HULL_EDGE_TABLE, new_buffer(cl_int2, hull_init));

        /* int
         * x: hull flags (bit-field)
         */
        set_buffer(HULL_FLAG, new_buffer(cl_int, hull_init));

        /* int
         * x: entity id for aligned hull
         */
        set_buffer(HULL_ENTITY_ID, new_buffer(cl_int, hull_init));

        /* int2
         * x: start bone
         * y: end bone
         */
        set_buffer(HULL_BONE_TABLE, new_buffer(cl_int2, hull_init));

        /* float
         * x: friction coefficient
         */
        set_buffer(HULL_FRICTION, new_buffer(cl_float, hull_init));

        /* float
         * x: restitution coefficient
         */
        set_buffer(HULL_RESTITUTION, new_buffer(cl_float, hull_init));

        /* int
         * x: reference mesh id
         */
        set_buffer(HULL_MESH_ID, new_buffer(cl_int, hull_init));

        /* int
         * x: offset index of the UV to use for this hull
         */
        set_buffer(HULL_UV_OFFSET, new_buffer(cl_int, hull_init));

        /* float2
         * x: initial reference angle
         * y: current rotation
         */
        set_buffer(HULL_ROTATION, new_buffer(cl_float2, hull_init));

        /* int
         * x: the integrity (i.e. health) of the hull
         */
        set_buffer(HULL_INTEGRITY, new_buffer(cl_int, hull_init));
         ...
         */
        set_buffer(ENTITY_ANIM_ELAPSED, new_buffer(cl_float2, entity_init));

        /* short2
         * x: number of ticks moving downward
         * y: number of ticks moving upward
         */
        set_buffer(ENTITY_MOTION_STATE, new_buffer(cl_short2, entity_init));

        /* int2
         * x: the currently running animation index
         * y: the previously running animation index
         */
        set_buffer(ENTITY_ANIM_INDEX, new_buffer(cl_int2, entity_init));
         ...
         */
        set_buffer(ENTITY_ROOT_HULL, new_buffer(cl_int, entity_init));

        /* int
         * x: model id of the aligned entity
         */
        set_buffer(ENTITY_MODEL_ID, new_buffer(cl_int, entity_init));

        /* int
         * x: model transform index of the aligned entity
         */
        set_buffer(ENTITY_TRANSFORM_ID, new_buffer(cl_int, entity_init));

        /* int2
         * x: start hull index
         * y: end hull index
         */
        set_buffer(ENTITY_HULL_TABLE, new_buffer(cl_int2, entity_init));

        /* int2
         * x: start bone anim index
         * y: end bone anim index
         */
        set_buffer(ENTITY_BONE_TABLE, new_buffer(cl_int2, entity_init));

        /* float
         * x: mass of the entity
         */
        set_buffer(ENTITY_MASS, new_buffer(cl_float, entity_init));
         ...
         */
        set_buffer(HULL_BONE, new_buffer(cl_float16, hull_init));

        /* int
         * x: bone bind pose index (model space)
         */
        set_buffer(HULL_BONE_BIND_POSE, new_buffer(cl_int, hull_init));

        /* int
         * x: bone inverse bind pose index (mesh-space)
         */
        set_buffer(HULL_BONE_INV_BIND_POSE, new_buffer(cl_int, hull_init));
         ...
         */
        set_buffer(ENTITY_BONE, new_buffer(cl_float16, entity_init));

        /* int
         * x: bind pose reference id
         */
        set_buffer(ENTITY_BONE_REFERENCE_ID, new_buffer(cl_int, entity_init));

        /* int
         * x: armature bone parent id
         */
        set_buffer(ENTITY_BONE_PARENT_ID, new_buffer(cl_int, entity_init));
