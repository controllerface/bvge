...
{
    public UnorderedSectorGroup(long ptr_queue)
    {
        super(ptr_queue);

        set_buffer(HULL_BONE,                new_buffer(CLSize.cl_float16));
        set_buffer(HULL_BONE_BIND_POSE,      new_buffer(CLSize.cl_int));
        set_buffer(HULL_BONE_INV_BIND_POSE,  new_buffer(CLSize.cl_int));

        set_buffer(ENTITY_BONE,              new_buffer(CLSize.cl_float16));
        set_buffer(ENTITY_BONE_REFERENCE_ID, new_buffer(CLSize.cl_int));
        set_buffer(ENTITY_BONE_PARENT_ID,    new_buffer(CLSize.cl_int));

        set_buffer(EDGE,                     new_buffer(CLSize.cl_int2));
        set_buffer(EDGE_LENGTH,              new_buffer(CLSize.cl_float));
        set_buffer(EDGE_FLAG,                new_buffer(CLSize.cl_int));

        set_buffer(POINT,                    new_buffer(CLSize.cl_float4));
        set_buffer(POINT_VERTEX_REFERENCE,   new_buffer(CLSize.cl_int));
        set_buffer(POINT_HULL_INDEX,         new_buffer(CLSize.cl_int));
        set_buffer(POINT_BONE_TABLE,         new_buffer(CLSize.cl_int4));
        set_buffer(POINT_HIT_COUNT,          new_buffer(CLSize.cl_short));
        set_buffer(POINT_FLAG,               new_buffer(CLSize.cl_int));

        set_buffer(HULL,                     new_buffer(CLSize.cl_float4));
        set_buffer(HULL_SCALE,               new_buffer(CLSize.cl_float2));
        set_buffer(HULL_POINT_TABLE,         new_buffer(CLSize.cl_int2));
        set_buffer(HULL_EDGE_TABLE,          new_buffer(CLSize.cl_int2));
        set_buffer(HULL_FLAG,                new_buffer(CLSize.cl_int));
        set_buffer(HULL_BONE_TABLE,          new_buffer(CLSize.cl_int2));
        set_buffer(HULL_ENTITY_ID,           new_buffer(CLSize.cl_int));
        set_buffer(HULL_FRICTION,            new_buffer(CLSize.cl_float));
        set_buffer(HULL_RESTITUTION,         new_buffer(CLSize.cl_float));
        set_buffer(HULL_MESH_ID,             new_buffer(CLSize.cl_int));
        set_buffer(HULL_UV_OFFSET,           new_buffer(CLSize.cl_int));
        set_buffer(HULL_ROTATION,            new_buffer(CLSize.cl_float2));
        set_buffer(HULL_INTEGRITY,           new_buffer(CLSize.cl_int));

        set_buffer(ENTITY_ANIM_ELAPSED,      new_buffer(CLSize.cl_float2));
        set_buffer(ENTITY_MOTION_STATE,      new_buffer(CLSize.cl_short2));
        set_buffer(ENTITY_ANIM_INDEX,        new_buffer(CLSize.cl_int2));
        set_buffer(ENTITY,                   new_buffer(CLSize.cl_float4));
        set_buffer(ENTITY_FLAG,              new_buffer(CLSize.cl_int));
        set_buffer(ENTITY_ROOT_HULL,         new_buffer(CLSize.cl_int));
        set_buffer(ENTITY_MODEL_ID,          new_buffer(CLSize.cl_int));
        set_buffer(ENTITY_TRANSFORM_ID,      new_buffer(CLSize.cl_int));
        set_buffer(ENTITY_HULL_TABLE,        new_buffer(CLSize.cl_int2));
        set_buffer(ENTITY_BONE_TABLE,        new_buffer(CLSize.cl_int2));
        set_buffer(ENTITY_MASS,              new_buffer(CLSize.cl_float));
    }
