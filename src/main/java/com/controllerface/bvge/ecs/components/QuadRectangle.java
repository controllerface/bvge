...
import com.controllerface.bvge.ecs.systems.physics.BoxKey;
...
public class QuadRectangle implements GameComponent
{
    ...
    private Set<BoxKey> keys = new HashSet<>();
    ...
    public void addkey(BoxKey key)
    {
        keys.add(key);

    }

    public Set<BoxKey> getKeys()
    {
        return keys;
    }
    ...
    {
        boolean thisNonZero = this.width > 0 && this.height > 0;
        boolean thatNonZero = r.width > 0 && r.height > 0;
        boolean xContained = r.x >= this.x && r.x + r.width <= this.x + this.width;
        boolean yContained = r.y >= this.y && r.y + r.height <= this.y + this.height;
        return thisNonZero && thatNonZero && xContained && yContained;
    }
